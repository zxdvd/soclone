...
class GithubOAuth2Mixin(OAuth2Mixin):
    ...
    def get_authenticated_user(self, redirect_uri, code, client_id,
        ...
        #github return types by default, return json string if set headers
        #properly
...
class WeiboOAuth2Mixin(OAuth2Mixin):
    ...
    _WEIBO_BASE_URL = 'https://api.weibo.com/2'
    _OAUTH_NO_CALLBACKS = False
    ...
    @_auth_return_future
    def weibo_request(self, full_url, callback, access_token=None,
            post_args=None, **args):
        """there is method oauth2_request in class OAuth2Mixin in dev branch of 
        Tornado (4.3). I copied the code here since dev branch not released"""
        all_args = {}
        if access_token:
            ...
            all_args.update(args)
        if all_args:
            full_url += "?" + urlencode(all_args)
        callback = functools.partial(self._on_weibo_request, callback)
        http = httpclient.AsyncHTTPClient()
        if post_args is not None:
            http.fetch(full_url, method="POST", body=urlencode(post_args),
                    callback=callback)
        else:
            http.fetch(full_url, callback=callback)
